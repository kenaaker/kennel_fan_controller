...
QString ZConfServiceClient::errorString() const { return avahi_strerror(error); }

ZConfServiceClient::ZConfServiceClient(QObject *parent)
    : QObject(parent), poll(avahi_qt_poll_get()), client(0), error(0) {}
...
void ZConfServiceClient::callback(AvahiClient *client, AvahiClientState state, void *userdata) {
    ...
    if (service) {
        ...
        case AVAHI_CLIENT_S_REGISTERING:
            qDebug() << (AVAHI_CLIENT_S_COLLISION == state ? "AVAHI_CLIENT_S_COLLISION" : "AVAHI_CLIENT_S_REGISTERING");
