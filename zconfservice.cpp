...
class ZConfServicePrivate {
  public:
    ZConfServicePrivate() : client(0), group(0), error(0) {}
...
ZConfService::ZConfService(QObject *parent) : QObject(parent), d_ptr(new ZConfServicePrivate) {
 ...
 */
bool ZConfService::isValid() const { return (d_ptr->group && !d_ptr->error); }
...
void ZConfService::registerService(QString name, in_port_t port, QString type) {
    ...
    if (!d_ptr->group) {
        d_ptr->group = avahi_entry_group_new(d_ptr->client->client, ZConfServicePrivate::callback, this);
    }
    if (avahi_entry_group_is_empty(d_ptr->group)) {
        d_ptr->error = avahi_entry_group_add_service(d_ptr->group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC,
                                                     (AvahiPublishFlags)0, d_ptr->name.toLatin1().data(),
                                                     d_ptr->type.toLatin1().data(), 0, 0, d_ptr->port, NULL);
        if (!d_ptr->error) {
 ...
 */
void ZConfService::resetService() { avahi_entry_group_reset(d_ptr->group); }
